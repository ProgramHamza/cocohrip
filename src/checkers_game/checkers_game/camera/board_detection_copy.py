...
class BoardDetection:
    ...
    def __init__(self, ximeaCamera=None, video_path=None):
        ...
        # If XimeaCamera is not available, use video
        if self.ximeaCamera is None:
            if self.video_path is not None:
                ...
                raise RuntimeError("No Ximea camera and no video file provided.")
        self._init()
    ...
    def get_camera_image(self):
        """
        Returns an image from XimeaCamera if available, otherwise from video file.
        """
        if self.ximeaCamera is not None:
            ...
            if not ret:
                ...
                return None
            return cv2.resize(frame, (600, 600))
        else:
            raise RuntimeError("No camera or video source available.")
    ...
    def _auto_detect_corners_from_image(self, image):
        if image is None:
            return None
        # Use only OldBoardBetterDetector's detect_corners_debug
    ...
    def _get_board_with_old_board_better(self, warped_board_image):
        # --- Begin new pipeline ---
        # 1. Detect corners (already done, so warped_board_image is warped)
        # 2. Piece recognition
        occupied_squares, square_stats = self.old_board_detector.piece_recognition(warped_board_image)
        # 3. Black/white assignment
        board, bw_debug = self.old_board_detector.define_bw_pieces(occupied_squares, square_stats)
        # 4. Build debug overlay
        overlay = warped_board_image.copy()
        h, w = overlay.shape[:2]
        cell_h = h // 8
        cell_w = w // 8
        for row in range(8):
        ...
        if (not self._old_board_runtime_reported) or (not self.is_initialized and black_count == 12 and white_count == 12):
            ...
            print(f"  Variance centers: {bw_debug['centers']}")
